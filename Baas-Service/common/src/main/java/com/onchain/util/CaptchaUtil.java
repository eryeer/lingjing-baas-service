...
public class CaptchaUtil {
    ...
    public static BufferedImage getImage(int w, int h, String code) {
        ...
        for (int i = 0; i < 15; i++) {
            // 设置颜色--随机数
            g2.setColor(new Color(random.nextInt(255), random.nextInt(255),
                    random.nextInt(255)));
            // 随机绘制先
            g2.drawLine(random.nextInt(w), random.nextInt(h),
                    random.nextInt(w), random.nextInt(h));
            // 随机点
            g2.drawOval(random.nextInt(w), random.nextInt(h), 25, 25);
        }*/
        ...
        g2.setFont(new Font("华文楷体", Font.ITALIC, fontSize));
